...
function ZGV:OnEnable()
	...
	self:AddEventHandler("QUEST_LOG_UPDATE")
	...
	if guide then
		...
		if guide.steps then
			...
			if stepobj then
				...
				if not silent then self:Print(L["message_loadedguide"]:format(name,step),false,"FORCE") end
	...
	if goal.parentStep.is_poi and ZGV.db.char.ActivatedPois[goal.parentStep.ident] then
		tinsert(menu,{
			...
			func = function()  
				...
				ZGV.Poi.DataProvider:RefreshAllData()
