...
do
	...
	if Lib then
				...
				for worldflighttype,data in ZGV.OrderedPairs(Lib.data.neighbourhood) do  repeat
					if worldflighttype~="neither" then
						...
						if world and ((Lib.speeds[world]>3) == (mode=="walk")) then
							-- trash it
							Lib:Debug("|cffff8888Tossed: %s",worldflighttype)
							table.wipe(data)
							break --continue
						end
					end
					...
					-- merge it!
					Lib:Debug("|cff88ff88Using: %s",worldflighttype)
		...
		function Lib:FindPath(am,ax,ay,bm,bx,by, handler, extradata, force_new, quiet)
			...
			self:Debug("&_PUSH |cff00ff88FindPath|r:  |cffaaff88%s|r##%d |cffffffff%.1f|r,|cffffffff%.1f|r --> |cffaaff88%s|r##%d/%d |cffffffff%.1f|r,|cffffffff%.1f|r",
		...
		function Lib:StepPath()  -- THE WORKHORSE.
			...
			for neigh,neighlink in current:IterNeighs() do
				...
				then
					...
					elseif current.type=="taxi" and current.known~=true then
		...
		function Lib:CheckMaxSpeeds()
			...
			for zoneid=1,2000 do
				...
				if (system and Lib.ZoneIsOutdoor(zoneid)) or flyable_subzones[zoneid] then
					local speed
					if system==MAPENUM["EASTERNKINGDOMS"] or system==MAPENUM["KALIMDOR"] or system==MAPENUM["DEEPHOLM"] then speed=Lib.speeds["Azeroth"]
					elseif system==MAPENUM["OUTLAND"] then speed=Lib.speeds["Outland"]
					elseif system==MAPENUM["NORTHREND"] then speed=Lib.speeds["Northrend"]
					elseif system==MAPENUM["PANDARIA"] then speed=Lib.speeds["Pandaria"]
					elseif system==MAPENUM["DRAENOR"] then speed=Lib.speeds["Draenor"]
					elseif system==MAPENUM["BROKENISLES"] then speed=Lib.speeds["Legion"]
					elseif system==MAPENUM["ARGUS"] then speed=Lib.speeds["Argus"]
					elseif system==MAPENUM["ZANDALAR"] then speed=Lib.speeds["Zandalar"]
					elseif system==MAPENUM["KULTIRAS"] then speed=Lib.speeds["KulTiras"]
					else speed=1.0 end
