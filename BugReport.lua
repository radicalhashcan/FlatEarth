...
function ZGV:GetBugReport(maint)
	...
	if BugReportFlags.player then
		...
		s = s .. ("Race: %s  Class: %s  Spec: %s  Level: %.2f   Faction: %s\n"):format(
			select(2,UnitRace("player")),select(2,UnitClass("player")), (select(2,GetSpecializationInfo(GetSpecialization() or 0))) or "none", self:GetPlayerPreciseLevel(),UnitFactionGroup("player"))
