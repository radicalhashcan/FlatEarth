...
function GearFinder:DisplayResults()
	...
	for i,data in ipairs(sorted_dungeons) do
		-- find guide for it 
		local dungeon_guide, dungeon = find_dungeon_guide(data[1])
	...
	if sorted_dungeons[1] then
		local dungeon = ZGV.Dungeons[sorted_dungeons[1][1]]
		MF.DungeonName:SetText(dungeon.name)
		MF.DungeonName:Show()
		MF.DungeonDesc:SetText(diff_to_name[dungeon.difficulty] .. "\n\n"..sorted_dungeons[1][2].." items found")
		MF.DungeonDesc:Show()
	end	
end
