	...
	for i=1,200 do
		...
		if name and bmin and (not header or headerhasrep) then
			...
			if	friendID==2010 or friendID==2011 or -- legion, season 3
				...
				bmin = 0
			elseif friendTexture and friendTexture==666623 then
				ranktype="bodyguard"
				val = friendRep - friendThresh
				bmax = min( friendMaxRep - friendThresh, 10000)
				bmin = 0
			elseif friendID then -- no texture, but ID is present: tiller friendship
				ranktype="friend"
				val = friendRep - friendThresh
			...
			local rep = self:CacheRep(name,standing,bmin,bmax,val,ranktype,friendThresh)
			rep.id = id
